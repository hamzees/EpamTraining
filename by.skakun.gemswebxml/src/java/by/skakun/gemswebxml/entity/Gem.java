...
public abstract class Gem {
     ...
     */
    public Gem(String id, int cost, double weight, int opacity, String color, String name) throws GemLogicException {
        setCost(cost);
        setWeight(weight);
        setOpacity(opacity);
        setColor(color);
...
public Gem() {
    ...
    @Override
        ...
        return "Название: " + name + ". ID: " + id +". Цена: " + cost + ". Вес: " + weight
     ...
     * @throws by.skakun.gemstones.exception.GemLogicException
     */
    public void setWeight(double weight) throws GemLogicException {
        if (weight > 0) {
            this.weight = weight;
        } else {
